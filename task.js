...
let shoppingList = async (lists, payment) => {         // create a function to buy a multiple books n times
    ...
    if(paymentStatus.toLowerCase() === "credit"){
        ...
        try {
            const reminderResult = await generateMonthlyBill(totalAll, creditMonth)
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            for (let x = 0; x < creditMonth - 1; x++) {
                if(x === targetMonth - 1){
                    monthlyBill.push(parseInt(Math.floor((finalTotal / creditMonth) / 1000) * 1000) + parseInt(newValue))
                }
                monthlyBill.push(Math.floor((finalTotal / creditMonth) / 1000) * 1000);
            }
...
// FUNGSI UNTUK MENAMBAHKAN TAGIHAN BUKU KE BULAN (X)

app.put('/tambah-tagihan/:index', async (req, res) => {
    ...
    if (!isNaN(oldValue) && !isNaN(newValue)) {
        ...
        try {
            ...
            const updatedBill = await generateMonthlyBill(finalTotal, finalCredit, index, newValue);
