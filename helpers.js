...
let shoppingList = async (lists, payment) => {         // create a function to buy a multiple books n times
    ...
    try {
        ...
        for (const bookLog of allBookLog) {
            const newLog = new BookModel({
                bookTitle: bookLog.title,
                quantity: bookLog.price,
                totalPrice: bookLog.genre,
                totalBill: bookLog.author,
            });

            newLog.save();
            console.log(`Book uploaded: ${bookLog.title}`)
        }
