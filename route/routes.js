...
const { bookList, manwhaList, loadingMessage, shoppingList, generateMonthlyBill, verifyToken, getNextSequenceValue, addBook } = require('../helper/helpers')

const { resetId, modifiedId} = require('../helper/changeId')
...
// ===================== YESTERDAY TASK => CRUD TO MONGODB =====================

router.get('/reset-id', async(req,res) => {
    try {
        await resetId()

        res.send(`Reset ID successfully.`)
...
// ADD BOOKLIST TO MONGO DB

router.get('/add-bookList', async(req, res) => {
    try {
        const addedBook = await addBook(bookList)

        res.json({ addedBook: addedBook})
    } catch (err) {
        res.status(500).send(err)
    }
})

// ADD 1 BOOK TO MONGO DB

router.post('/add-book', async(req, res) => {
    try {

        const { title, price, genre, author, stock, purchased } = req.body
...
// SHOW ALL BOOK IN MONGO DB

router.get('/show-book', async(req, res) => {
    try {
...
router.put('/edit-book/:id', async(req, res) => {
    try {
        ...
        await Book.findByIdAndUpdate(id, { title, price, genre, author, stock, purchased }, { new:true })
...
// ===================== TODAY TASK => CREATE BOOKSHELVES COLLECTION =====================

router.get('/add-manwhaList', async(req, res) => {
    try {
        const addedBook = await addBook(manwhaList)
...
router.post('/add-bookshelves', async(req, res) => {
    try {
        
        const { name, description, book } = req.body

        const newBookshelves = new Bookshelves({
            name,
            description,
            book
        })
        ...
        res.send(`Bookshelves added succesfully`)
...
router.get('/show-bookshelves', async(req, res) => {
    try {
        ...
        if(!bookIds){
            ...
            try {
                const bookshelves = await Bookshelves.find().populate('book')
            ...
            try {
                const bookshelves = await Bookshelves.find( { book: { $in: bookIdsArray } } ).populate('book')
                res.json(bookshelves)
            } catch (err) {
                res.status(500).send(err)
            }
...
router.put('/edit-bookshelves/:id', async(req, res) => {
    try {
        ...
        await Bookshelves.findByIdAndUpdate(id, { name, description, book }, { new:true })

        res.send(`Bookshelves updated successfully`)
...
router.delete('/delete-bookshelves/:id', async(req, res) => {
    try {
        
        const {id} = req.params

        const deletedBookshelves = await Bookshelves.findByIdAndDelete(id)

        if(!deletedBookshelves){
            return res.status(404).send(`Bookshelves not found`)
        }

        res.send(`Bookshelves deleted successfully.`)
